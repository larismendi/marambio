<?php
	...
	$db = new mysqli('localhost', 'root' ,'', 'marambio3');
	...
	if(!$db) {
		...
		if(isset($_POST['queryString'])) {
			...
			if(strlen($queryString) >0) {
				...
				$query = $db->query("SELECT Nombre FROM usuarios WHERE Usuario LIKE '$queryString%'");
				if($query) {
					...
					while ($result = $query ->fetch_object()) {
						...
						// YOU MUST CHANGE: $result->value to $result->your_colum
	         			echo '<li onClick="fill(\''.$result->Nombre.'\');">'.$result->Nombre.'</li>';
	         		}
				} else {
